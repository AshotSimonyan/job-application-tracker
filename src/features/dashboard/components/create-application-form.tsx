    ...
    <section className="border-line bg-surface rounded-[2rem] border p-6 shadow-[0_24px_72px_-64px_rgba(8,22,47,0.28)] sm:p-8">
      <div>
        ...
        <p className="text-muted mt-3 max-w-3xl text-sm leading-7 sm:text-base">
          Add a role to your tracker and keep the dashboard aligned with your
          current search.
        </p>
      </div>
      ...
      {result ? (
        <div
          className={cn(
            "mt-6 rounded-[1.5rem] border px-4 py-3 text-sm",
            result.status === "error"
      ...
      <form onSubmit={onSubmit} className="mt-6 grid gap-4" noValidate>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="text-foreground grid gap-2 text-sm font-medium">
            ...
            <input
              type="text"
              placeholder="Remote"
              className="border-line bg-surface-alt text-foreground placeholder:text-muted/70 focus:border-brand h-12 rounded-2xl border px-4 text-sm transition-colors outline-none"
              {...register("location")}
            />
          </label>
          ...
          <label className="text-foreground grid gap-2 text-sm font-medium">
            Status
            <select
              className="border-line bg-surface-alt text-foreground focus:border-brand h-12 rounded-2xl border px-4 text-sm transition-colors outline-none"
              {...register("status")}
            >
        ...
        <label className="text-foreground grid gap-2 text-sm font-medium">
          ...
          <textarea
            ...
            className="border-line bg-surface-alt text-foreground placeholder:text-muted/70 focus:border-brand rounded-[1.5rem] border px-4 py-3 text-sm transition-colors outline-none"
            {...register("notes")}
          />
        </label>
        ...
        <div className="flex items-center justify-between gap-4">
          <p className="text-muted text-sm">
            This creates a single application record for the current user.
          </p>
          <button
            type="submit"
            disabled={isBusy}
